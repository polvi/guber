...
export const patchApiV1NamespacesName = async (
  ...
  const res = await fetch(getPatchApiV1NamespacesNameUrl(name, params), {
    ...options,
    method: "PATCH",
  ...
  const data: patchApiV1NamespacesNameResponse["data"] = body
    ? JSON.parse(body)
    : {};
  return {
    data,
    status: res.status,
    headers: res.headers,
  } as patchApiV1NamespacesNameResponse;
};
