...
import {
  ...
  listWorker,
  getWorker,
  listWorkerScriptVersion,
  getWorkerScriptVersion,
  listWorkerScriptDeployment,
  getWorkerScriptDeployment,
  listD1,
  getD1,
  listQueue,
  getQueue,
...
class CloudflareController implements Controller {
    ...
    for (const resource of results || []) {
      try {
        ...
        if (
          ...
          if (hasDependency) {
            console.log(
              `Found dependent resource ${resource.metadata.name}, checking if all dependencies are now ready`,
            );
            ...
            if (allDependenciesReady) {
              ...
              console.log(
                `⏳ ${resource.kind} ${resource.metadata.name} still has unresolved dependencies:`,
              ...
              const updatedStatus = {
                ...status,
                state: "Pending",
                message: `Waiting for dependencies: ${unresolvedDependencies
                  .map((d) => `${d.kind}/${d.name}`)
                  .join(", ")}`,
  ...
  private async reconcileQueues(env: any) {
    try {
      ...
      const apiResourcesResponse = await listQueue();
      ...
      const cloudflareResult = (await cloudflareResponse.json()) as any;
      ...
      for (const { fullName, resource } of missingQueues) {
        try {
          ...
          if (createResponse.ok) {
            ...
            await patchQueue(resource.name, {
              apiVersion: "cf.guber.proc.io/v1",
              kind: "Queue",
  ...
  private async provisionWorker(
    ...
    try {
      ...
      if (spec.bindings) {
        ...
        if (spec.bindings.d1_databases) {
          for (const d1Binding of spec.bindings.d1_databases) {
            ...
            let d1Resource = null;
            try {
              d1Resource = await getD1(d1Binding.database_name);
            } catch (error) {
              d1Resource = null;
            }
        ...
        if (spec.bindings.queues) {
          for (const queueBinding of spec.bindings.queues) {
            ...
            try {
              queueResource = await getQueue(queueBinding.queue_name);
            } catch (error) {
              queueResource = null;
            }
      ...
      if (!domainResponse.ok) {
        ...
        await patchWorker(resourceName, {
          apiVersion: "cf.guber.proc.io/v1",
          kind: "Worker",
          metadata: { name: resourceName },
          status: {
      ...
      const domainResult = (await domainResponse.json()) as any;
      console.log(
  ...
  private async reconcileWorkers(env: any) {
    try {
      ...
      const apiResourcesResponse = await listWorker();
      const apiResources = apiResourcesResponse.data.items || [];
      ...
      const workersResult = (await workersResponse.json()) as any;
      const domainsResult = (await domainsResponse.json()) as any;
      ...
      for (const resource of apiResources || []) {
        const fullWorkerName = this.buildFullDatabaseName(
          resource.metadata?.name || (resource as any).name,
          "cf.guber.proc.io",
          "workers",
          resource.metadata?.namespace || (resource as any).namespace,
          env.GUBER_NAME,
        );
      ...
      for (const { fullName, resource } of missingWorkers) {
        try {
          ...
          if (createResponse.ok) {
            ...
            await patchWorker(resource.name, {
              apiVersion: "cf.guber.proc.io/v1",
              kind: "Worker",
              metadata: { name: resource.name },
              status: statusData,
          ...
          try {
            ...
            if (spec.bindings) {
                  ...
                  if (
                    ...
                    if (queueStatus.queue_id) {
                      const fullQueueName = this.buildFullDatabaseName(
                        ...
                        "cf.guber.proc.io",
                        "qs",
                        (queueResource as any).metadata?.namespace ||
              ...
              if (workerResponse.ok) {
                ...
                currentBindings = workerData.result?.bindings || [];
              } else {
            ...
            if (needsBindingUpdate) {
              ...
              if (updateResponse.ok) {
                ...
                await patchWorker(apiResource.name, {
                  apiVersion: "cf.guber.proc.io/v1",
                  kind: "Worker",
            ...
            try {
              ...
              const errorMessage =
                (healthError as Error).message || String(healthError);
            ...
            const newStatus = {
              ...
              state: "Failed",
              lastHealthCheck: new Date().toISOString(),
              healthCheckError: `Worker check failed: ${
  ...
  private async reconcilePendingWorkers(env: any) {
    ...
    const workersResponse = await listWorker();
    ...
    for (const worker of pendingWorkers || []) {
      try {
        ...
        if (
          ...
          for (const dependency of spec.dependencies) {
            ...
            try {
              if (dependency.kind === "Worker") {
                ...
                depResource = await getWorkerScriptDeployment(dependency.name);
              } else if (dependency.kind === "D1") {
                depResource = await getD1(dependency.name);
              } else if (dependency.kind === "Queue") {
                depResource = await getQueue(dependency.name);
            ...
            const depStatus = depResource.data.status as any;
          ...
          if (allDependenciesReady) {
            console.log(
              `[Reconcile] All dependencies resolved for worker ${worker.metadata.name}, re-queuing for provisioning`,
            );
            ...
            if (env.GUBER_BUS) {
              await env.GUBER_BUS.send({
                action: "create",
                resourceType: "worker",
                resourceName: worker.metadata.name,
                group: worker.apiVersion.split("/")[0],
                kind: worker.kind,
                plural: worker.kind.toLowerCase() + "s",
                namespace: worker.metadata.namespace,
                spec: spec,
              });
            }
          } else {
            console.log(
              `[Reconcile] Worker ${worker.metadata.name} still has unresolved dependencies:`,
              unresolvedDependencies.map((d) => `${d.kind}/${d.name}`),
            );
            ...
            await patchWorker(worker.metadata.name!, {
              apiVersion: "cf.guber.proc.io/v1",
              kind: "Worker",
              metadata: { name: worker.metadata.name },
              status: updatedStatus,
            });
          }
        }
        ...
        console.error(
          `[Reconcile] Error checking dependencies for worker ${worker.metadata.name}:`,
          error,
        );
      }
    }
  ...
  private async reconcileD1Databases(env: any) {
    try {
      ...
      const apiResourcesResponse = await listD1();
      const apiResources = apiResourcesResponse.data.items || [];
  ...
  private async provisionWorkerScriptVersion(
    ...
    try {
      ...
      try {
        workerResource = await getWorker(spec.scriptName);
      } catch (error) {
        workerResource = null;
      }
      ...
      const actualScriptName = (workerStatus.endpoint as string).split(
        "/workers/scripts/",
      )[1];
      ...
      const workerScriptVersionUpdate: WorkerScriptVersion = {
        ...
        status: {
          ...
          metadata: (result.result.metadata as any) || {},
        },
      };
  ...
  private async reconcileWorkerScriptVersions(env: any) {
    try {
      ...
      const apiResourcesResponse = await listWorkerScriptVersion();
      const apiResources = apiResourcesResponse.data.items || [];
        ...
        try {
          ...
          if (
            ...
            if (status.pendingWorker) {
              ...
              try {
                targetWorkerResource = await getWorker(status.pendingWorker);
              } catch (error) {
              ...
              if (
                ...
                if (targetWorkerStatus.state === "Ready") {
                  console.log(
                    `[Reconcile] Target worker ${status.pendingWorker} is now ready for version ${resource.metadata.name}, re-queuing for provisioning`,
                  );
                  ...
                  console.log(
                    `[Reconcile] Target worker ${status.pendingWorker} still not ready (${targetWorkerStatus.state}) for version ${resource.metadata.name}`,
                  );
                ...
                console.log(
                  `[Reconcile] Target worker ${status.pendingWorker} not found or has no status for version ${resource.metadata.name}`,
                );
            ...
            if (spec && spec.dependencies && Array.isArray(spec.dependencies)) {
              let allDependenciesReady = true;
              const unresolvedDependencies = [];
              ...
              if (allDependenciesReady) {
                console.log(
                  `[Reconcile] All dependencies resolved for worker script version ${resource.metadata.name}, re-queuing for provisioning`,
                );
                ...
                console.log(
                  `[Reconcile] Worker script version ${resource.metadata.name} still has unresolved dependencies:`,
                  unresolvedDependencies.map((d) => `${d.kind}/${d.name}`),
                );
          ...
          if (
            ...
            try {
              ...
              if (!versionResponse.ok) {
                console.log(
                  `Worker script version ${resource.metadata.name} (ID: ${status.version_id}) no longer exists in Cloudflare`,
                );
  ...
  private async provisionWorkerScriptDeployment(
    ...
    try {
      ...
      // Update status to failed
      await patchWorkerScriptDeployment(resourceName, {
        apiVersion: "cf.guber.proc.io/v1",
        kind: "WorkerScriptDeployment",
        metadata: { name: resourceName },
        status: {
  ...
  private async reconcileWorkerScriptDeployments(env: any) {
    try {
      ...
      const apiResourcesResponse = await listWorkerScriptDeployment();
      const apiResources = apiResourcesResponse.data.items || [];
            ...
            if (status.pendingWorker) {
              ...
              if (
                ...
                if (targetWorkerStatus.state === "Ready") {
                  ...
                  await patchWorkerScriptDeployment(resource.name, {
                    apiVersion: "cf.guber.proc.io/v1",
                    kind: "WorkerScriptDeployment",
                    metadata: { name: resource.name },
                    status: {
