    ...
    for (const message of batch.messages) {
      try {
        ...
        if (action === "create") {
          ...
          if (resourceType === "d1") {
            provisioningSuccessful = await provisionD1Database(
              env,
              resourceName,
              group,
              kind,
            ...
            provisioningSuccessful = await provisionQueue(
              env,
              resourceName,
              group,
              kind,
            ...
            provisioningSuccessful = await provisionWorker(
              env,
              resourceName,
              group,
              kind,
          ...
          if (provisioningSuccessful) {
            await checkAndProvisionDependentResources(
              env,
              resourceName,
              group,
              kind,
...
async function checkAndProvisionDependentResources(
  env: Env,
...
function buildFullDatabaseName(
  resourceName: string,
  group: string,
  plural: string,
...
async function provisionD1Database(
  env: Env,
  resourceName: string,
  group: string,
  kind: string,
  ...
  const fullDatabaseName = buildFullDatabaseName(
    resourceName,
    group,
    plural,
    namespace,
...
async function provisionQueue(
  ...
  const fullQueueName = buildFullDatabaseName(
    resourceName,
    group,
    plural,
    namespace,
...
async function provisionWorker(
  ...
  const fullWorkerName = buildFullDatabaseName(
    resourceName,
    group,
    plural,
    namespace,
